...
\hlstd{heating_long} \hlkwb{<-} \hlstd{heating_long} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{constant_ec} \hlstd{=} \hlnum{1} \hlopt{*} \hlstd{(alt} \hlopt{==} \hlstr{'ec'}\hlstd{),}
         \hlkwc{constant_er} \hlstd{=} \hlnum{1} \hlopt{*} \hlstd{(alt} \hlopt{==} \hlstr{'er'}\hlstd{),}
         \hlkwc{constant_gc} \hlstd{=} \hlnum{1} \hlopt{*} \hlstd{(alt} \hlopt{==} \hlstr{'gc'}\hlstd{),}
         \hlkwc{constant_gr} \hlstd{=} \hlnum{1} \hlopt{*} \hlstd{(alt} \hlopt{==} \hlstr{'gr'}\hlstd{))}
\end{alltt}
\end{kframe}
\end{knitrout}
\end{frame}
...
\hlstd{heating_long} \hlopt{%>%}
  \hlkwd{select}\hlstd{(idcase, alt, choice, ic, oc,} \hlkwd{starts_with}\hlstd{(}\hlstr{'constant'}\hlstd{))}
\end{alltt}
\begin{verbatim}
## # A tibble: 4,500 x 9
##    idcase alt   choice    ic    oc constant_ec constant_er constant_gc
##     <dbl> <chr> <lgl>  <dbl> <dbl>       <dbl>       <dbl>       <dbl>
##  1      1 ec    FALSE   860.  553.           1           0           0
##  2      1 er    FALSE   996.  506.           0           1           0
##  3      1 gc    TRUE    866   200.           0           0           1
##  4      1 gr    FALSE   963.  152.           0           0           0
##  5      1 hp    FALSE  1136.  238.           0           0           0
##  6      2 ec    FALSE   797.  520.           1           0           0
##  7      2 er    FALSE   895.  486.           0           1           0
##  8      2 gc    TRUE    728.  169.           0           0           1
##  9      2 gr    FALSE   759.  169.           0           0           0
## 10      2 hp    FALSE   969.  199.           0           0           0
## # ... with 4,490 more rows, and 1 more variable: constant_gr <dbl>
\end{verbatim}
\end{kframe}
\end{knitrout}
\end{frame}
...
\hlstd{ll_fn_2} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{params}\hlstd{,} \hlkwc{data}\hlstd{)\{}
  ...
  \hlstd{X} \hlkwb{<-} \hlstd{data} \hlopt{%>%}
    \hlkwd{select}\hlstd{(}\hlkwd{starts_with}\hlstd{(}\hlstr{'constant'}\hlstd{), ic, oc)} \hlopt{%>%}
    \hlkwd{as.matrix}\hlstd{()}
  \hlcom{## Calculate representative utility for each alternative given the parameters}
  \hlstd{model_data} \hlkwb{<-} \hlstd{data} \hlopt{%>%}
    \hlkwd{mutate}\hlstd{(}\hlkwc{utility} \hlstd{= X} \hlopt{%*%} \hlstd{params)}
...
\end{alltt}
\begin{verbatim}
## [1] 0.4450210322 0.3584427563 0.2263707192 0.2060168049 0.0006199767
## [6] 0.0015351975
\end{verbatim}
\end{kframe}
\end{knitrout}
