...
\hlkwd{c}\hlstd{(model_1c}\hlopt{$}\hlstd{coefficients[}\hlnum{3}\hlstd{]} \hlopt{/ -}\hlstd{model_1c}\hlopt{$}\hlstd{coefficients[}\hlnum{4}\hlstd{]} \hlopt{*} \hlnum{0.1}\hlstd{,}
  \hlkwd{abs}\hlstd{(model_1c}\hlopt{$}\hlstd{coefficients[}\hlnum{7}\hlstd{])} \hlopt{/ -}\hlstd{model_1c}\hlopt{$}\hlstd{coefficients[}\hlnum{4}\hlstd{]} \hlopt{*} \hlnum{0.1}\hlstd{)} \hlopt{%>%}
...
\hlstd{data_1} \hlopt{%>%}
  ...
  \hlkwd{summarize}\hlstd{(}\hlkwc{apple} \hlstd{=} \hlkwd{mean}\hlstd{(apple),}
            \hlkwc{storage} \hlstd{=} \hlkwd{mean}\hlstd{(storage),}
            \hlkwc{screen} \hlstd{=} \hlkwd{mean}\hlstd{(screen))} \hlopt{%>%}
...
\hlstd{data_2b} \hlopt{%>%}
  ...
  \hlkwd{summarize}\hlstd{(}\hlkwc{apple_coef} \hlstd{=} \hlkwd{mean}\hlstd{(apple_coef),}
            \hlkwc{storage_coef} \hlstd{=} \hlkwd{mean}\hlstd{(storage_coef),}
            \hlkwc{screen_coef} \hlstd{=} \hlkwd{mean}\hlstd{(screen_coef))} \hlopt{%>%}
  \hlkwd{ungroup}\hlstd{()} \hlopt{%>%}
