...
\hlstd{data_multi} \hlkwb{<-} \hlstd{data_multi} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{elas_own_car_cost_3a} \hlstd{=}
           \hlkwd{coef}\hlstd{(model_3a)[}\hlnum{4}\hlstd{]} \hlopt{*} \hlstd{cost.car} \hlopt{*} \hlstd{(}\hlnum{1} \hlopt{-} \hlstd{prob_car_3a))}
\hlcom{## Calculate the cross-elasticity of car cost}
\hlstd{data_multi} \hlkwb{<-} \hlstd{data_multi} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{elas_cross_car_cost_3a} \hlstd{=}
           \hlopt{-}\hlkwd{coef}\hlstd{(model_3a)[}\hlnum{4}\hlstd{]} \hlopt{*} \hlstd{cost.car} \hlopt{*} \hlstd{prob_car_3a)}
...
\hlstd{model_3b_single} \hlkwb{<-} \hlkwd{mlogit}\hlstd{(}\hlkwc{formula} \hlstd{= mode} \hlopt{~} \hlstd{cost} \hlopt{|} \hlnum{1} \hlopt{|} \hlstd{time,}
                          \hlkwc{data} \hlstd{= data_dfidx_single)}
\hlcom{## Model choice for single students}
\hlstd{model_3b_married} \hlkwb{<-} \hlkwd{mlogit}\hlstd{(}\hlkwc{formula} \hlstd{= mode} \hlopt{~} \hlstd{cost} \hlopt{|} \hlnum{1} \hlopt{|} \hlstd{time,}
...
\hlkwd{abs}\hlstd{(}\hlkwd{coef}\hlstd{(model_3b_single)[}\hlnum{5}\hlopt{:}\hlnum{8}\hlstd{]} \hlopt{/} \hlkwd{coef}\hlstd{(model_3b_single)[}\hlnum{4}\hlstd{])} \hlopt{*} \hlnum{60}
\end{alltt}
\begin{verbatim}
...
\hlkwd{abs}\hlstd{(}\hlkwd{coef}\hlstd{(model_3b_married)[}\hlnum{5}\hlopt{:}\hlnum{8}\hlstd{]} \hlopt{/} \hlkwd{coef}\hlstd{(model_3b_married)[}\hlnum{4}\hlstd{])} \hlopt{*} \hlnum{60}
\end{alltt}
\begin{verbatim}
## time:bike  time:bus  time:car time:walk 
